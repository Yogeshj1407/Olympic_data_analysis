...
if user_menu == 'Medal Tally':
    ...
    if selected_country != 'Overall' and selected_year != 'Overall':
        st.header('Medal Tally of '+ selected_country + ' in ' + str(selected_year) + ' Olympics')
...
if user_menu == 'Overall Analysis':
    ...
    st.title('No of Events over the Year - Every Sport')
...
if user_menu == 'Country-wise Analysis':
    ...
    pivot_df = helper.country_event_heatmap(df,selected_country)
    fig, ax = plt.subplots(figsize=(25, 25))
    ax = sns.heatmap(pivot_df,annot = True)
    st.pyplot(fig)

    st.title('Top 10 Athletes Country-wise')
    top_ten_df = helper.most_successful_athletes_countrywise(df,selected_country)
    st.table(top_ten_df)
...
if user_menu == 'Athlete-wise Analysis':
    ...
    # x,name = helper.famous_sport_age_distribution(df)
    ...
    famous_sports = ['Basketball', 'Judo', 'Football', 'Tug-Of-War', 'Athletics', 'Swimming',
                     ...
                     'Cycling', 'Diving', 'Canoeing', 'Tennis', 'Golf', 'Archery', 'Softball', 'Volleyball',
    ...
    st.title('Distribution of Age Over Sports (for Gold Medal)')
    fig.update_layout(autosize=False, width=1000, height=600)
    st.plotly_chart(fig)
